...
public final class ScrollableListGui {
    ...
    public static void open(ServerPlayer player, CompoundTag data, String variant) {
        if (requiresManagement(variant) && !canManageTargetIsland(player, data)) return;
        ...
        switch (variant) {
            ...
            case "biomes" -> {
                if (itemData.contains("biome")) {
                    ...
                    if (island != null) {
                        if (IslandAdministrationService.updateBiome(
                                player, island, itemData.getString("biome"))) {
                ...
                if (itemData.contains("group_id") && data.contains("player_id")) {
                    ...
                    if (island != null) {
                        ...
                        if (targetGroup != null) {
                            ...
                            boolean assigned = IslandAdministrationService.assignMemberGroup(
                                    player, island, playerId, targetGroup.getId());
                            if (assigned) {
    ...
    private static boolean requiresManagement(final String variant) {
        return "biomes".equals(variant)
                || "groups".equals(variant)
                || "members_group".equals(variant)
                || "set_group".equals(variant);
    }

    private static boolean canManageTargetIsland(final ServerPlayer player, final CompoundTag data) {
        if (!data.contains("island_id")) return false;
        final Island island = IslandManager.getInstance().getIslandByUUID(data.getUUID("island_id"));
        return EnhancedGuiHelper.canManageIsland(player, island);
    }
    ...
    private static List<ItemStack> buildBiomeItems(ServerPlayer player, CompoundTag data) {
        ...
        while (biomeRegistry.hasNext()) {
            ...
            if (iconItem.isPresent()) {
                net.minecraft.world.item.Item resolved = ForgeRegistries.ITEMS.getValue(
                        ResourceLocation.parse(iconItem.get())
                );
